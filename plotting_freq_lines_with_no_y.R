...
#it was done by creating a "table" object from a vector of dates, and plotting a 'y=' as the frequency count of unique dates in the table.  

#Note: answering this question is part of larger project, seems like oversharing is better than under. SEE LINES 51:71 for solution. 

#relevant packages used.
library(ggplot2)
library(dplyr)

#bringing in the dataset. These are crowdsourced community and public health reports from mobile devices. nrow here is around 4 * 10^6 and ncol = 23.
...
#date range of, this dataset goes back to 2008. The ask in this situation was for 2016 - 2020.
...
# Note: these lines apply to the R-geospatial component of this project (need clean coordinates within a range or the map breaks.) Part of a larger workflow.
sanfran_range_recent_calls <- subset(recent_waste_calls[recent_waste_calls$lat > 37, ])

#filtering out NA values in lat/lon columns with dplyr. Only works if all the way clean..
sanfran_calls_no_na = filter(sanfran_range_recent_calls, lat != "NA")


### --- The next few lines are methods to control for confounding in the data (mostly duplicates of where a Public Health Worker shows up and nothing is there to fix. This is some noisy data, folks! Bear with me... ) --- ###


#filter out "duplicate calls" Note: many duplicates documented through San Francisco 311 dataset.
sanfran_clean_no_na_1 <- sanfran_calls_no_na[grep("Duplicate", sanfran_calls_no_na$status_notes, invert = TRUE), ]

#Duplicates are documented on different columns in the dataset, see "status_notes" and "agency_responsible".
...
#Okay, now these lines use a gsub() function to make a new vector (column) in the dataframe with a cleaned up and ggplot2 recognizable "date" format: YYYY-MM-DD without timestamp. That argument in the parantheses is R-regex code.
sanfran_clean_calls$date_only <- gsub('[[:upper:]][0-9][^z][[:punct:]][0-9][^z][[:punct:]][0-9][^z][[:upper:]]', '', sanfran_clean_calls$requested_datetime)

# And finally, a dplyr function to clear out duplicates based on address, (those one incidence, multiple reports situations where 2+ people see something and report it at the same time).
sanfran_map_these = sanfran_clean_calls %>% distinct(address, .keep_all = TRUE)

#Note: the date_only vector is just that, a single column of dates reflecting each time a user reported human or animal waste to San Francisco's 311 system between 2016 - 2020 (nrow is about 3.03 * 10^4).

#building a table out of our new "date_only" vector. 
y_dates <- as.data.frame(table(sanfran_map_these$date_only))


#frequency plot with line. Works with table parameters!
