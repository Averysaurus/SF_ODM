...
# 100 Valencia Street, June 15th, 2016 
...
# Setting time frame before/after June 15th 2016: 
...
valencia_box_bef <- subset(odm_coords_bef, 
                          point.latitude >= 37.7697523948 &    # lower lat bound
                          point.latitude <= 37.773768903 &    # upper lat bound
                          point.longitude >= -122.4252524256 &  # upper long bound
                          point.longitude <= -122.4201516602)   # lower long bound

# 2 cumulative incidence one month before 
tmap_mode('view')
tm_shape(odm_soma) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(valencia_box_bef) +
  tm_dots(col = 'red', size = 0.01) 


valencia_box_aft <- subset(odm_coords_aft, 
                    point.latitude >= 37.7697523948 &    # lower lat bound
                    point.latitude <= 37.773768903 &    # upper lat bound
                    point.longitude >= -122.4252524256 &  # upper long bound
                    point.longitude <= -122.4201516602)   # lower long bound
...
# 7-8 cumulative incidence 1 month after.
tmap_mode('view')
tm_shape(odm_soma) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(valencia_box_aft) +
  tm_dots(col = 'red', size = 0.01) 
...
valencia_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                          point.latitude >= 37.7697523948 &    # lower lat bound
                          point.latitude <= 37.773768903 &    # upper lat bound
                          point.longitude >= -122.4252524256 &  # upper long bound
                          point.longitude <= -122.4201516602)   # lower long bound

# 1 cumulative incidence (2 weeks)
tmap_mode('view')
tm_shape(odm_soma) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(valencia_box_bef_twowk) +
  tm_dots(col = 'red', size = 0.01) 


valencia_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                      point.latitude >= 37.7697523948 &    # lower lat bound
                      point.latitude <= 37.773768903 &    # upper lat bound
                      point.longitude >= -122.4252524256 &  # upper long bound
                      point.longitude <= -122.4201516602)   # lower long bound
...
tm_shape(odm_soma) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(valencia_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01) 


### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(valencia_box_bef)
raw_incidence_aft <- st_drop_geometry(valencia_box_aft)
