...
# Civic Center: Grove and Larkin, July 1st, 2015 
# type listed as JC Decaux
##ERROR : can only locate data back to 2016 : review code again??
# range(sanfran_calls$requested_datetime)
...
not_medical_waste <- waste_calls[waste_calls$service_subtype 
                                 != "Medical Waste", ]
...
recent_waste_calls <- subset(not_medical_waste, 
                    as.data.frame.Date(requested_datetime) >= "2015-01-01" & 
...
sanfran_range_recent_calls <- 
  subset(recent_waste_calls[recent_waste_calls$lat > 37, ])
...
odm_civic <- sf_nhoods[sf_nhoods$name == 'Civic Center', ]

# -122.420249447,37.7762363126,-122.41170603,37.7810768404
#bounding box for toilet before (1 month). 
grovelarkin_box_bef <- subset(odm_coords_bef, 
                    point.latitude >= 37.7762363126 &    # lower lat bound
                    point.latitude <= 37.7810768404 &    # upper lat bound
                    point.longitude >= -122.420249447 &  # upper long bound
                    point.longitude <= -122.41170603)   # lower long bound

# 4 cumulative incidence one month before. 
tmap_mode('view')
tm_shape(odm_civic) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(grovelarkin_box_bef) +
  tm_dots(col = 'red', size = 0.01) 


grovelarkin_box_aft <- subset(odm_coords_aft, 
                      point.latitude >= 37.7762363126 &    # lower lat bound
                      point.latitude <= 37.7810768404 &    # upper lat bound
                      point.longitude >= -122.420249447 &  # upper long bound
                      point.longitude <= -122.41170603)   # lower long bound
...
grovelarkin_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                        point.latitude >= 37.7762363126 &    # lower lat bound
                        point.latitude <= 37.7810768404 &    # upper lat bound
                        point.longitude >= -122.420249447 &  # upper long bound
                        point.longitude <= -122.41170603)   # lower long bound
...
# 3 cumulative incidence (2 weeks)
tmap_mode('view')
tm_shape(odm_civic) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(grovelarkin_box_bef_twowk) +
  tm_dots(col = 'red', size = 0.01) 


grovelarkin_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                        point.latitude >= 37.7762363126 &    # lower lat bound
                        point.latitude <= 37.7810768404 &    # upper lat bound
                        point.longitude >= -122.420249447 &  # upper long bound
                        point.longitude <= -122.41170603)   # lower long bound
...
# 3 cumulative incidence after, (2 wweks).
tmap_mode('view')
tm_shape(odm_civic) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(grovelarkin_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01)


### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(grovelarkin_box_bef)
raw_incidence_aft <- st_drop_geometry(grovelarkin_box_aft)
