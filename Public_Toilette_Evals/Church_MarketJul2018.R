-122.434624,37.762448,-122.421329,37.771545

### 4.27.20 ###
# odm toilette intervention chunk.
# Civic Center: Church & Market, July 1st, 2018 
...
odm_duboce <- sf_nhoods[sf_nhoods$name == 'Duboce Triangle', ]
odm_dolores <- sf_nhoods[sf_nhoods$name == 'Mission Dolores', ]

#bounding box for toilet before (1 month). 
marketchurch_box_bef <- subset(odm_coords_bef, 
                        point.latitude >= 37.762448 &    # lower lat bound
                        point.latitude <= 37.771545 &    # upper lat bound
                        point.longitude >= -122.434624 &  # upper long bound
                        point.longitude <= -122.4213293)   # lower long bound

# 25 cumulative incidence one month before. 
tmap_mode('view')
tm_shape(odm_duboce) +
  tm_polygons(border.col = 'blue', alpha = 0) +
tm_shape(odm_dolores) +
  tm_polygons(border.col = 'blue', alpha = 0) +
tm_shape(marketchurch_box_bef) +
  tm_dots(col = 'red', size = 0.01) 


marketchurch_box_aft <- subset(odm_coords_aft, 
                        point.latitude >= 37.762448 &    # lower lat bound
                        point.latitude <= 37.771545 &    # upper lat bound
                        point.longitude >= -122.434624 &  # upper long bound
                        point.longitude <= -122.4213293)   # lower long bound
...
# 28 cumulative incidence 1 month after.
tmap_mode('view')
tm_shape(odm_duboce) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(odm_dolores) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(marketchurch_box_aft) +
  tm_dots(col = 'red', size = 0.01)
...
marketchurch_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                    point.latitude >= 37.762448 &    # lower lat bound
                    point.latitude <= 37.771545 &    # upper lat bound
                    point.longitude >= -122.434624 &  # upper long bound
                    point.longitude <= -122.4213293)   # lower long bound

# 16 cumulative incidence (2 weeks)
tmap_mode('view')
tm_shape(odm_duboce) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(odm_dolores) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(marketchurch_box_bef_twowk) +
  tm_dots(col = 'red', size = 0.01)


marketchurch_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                          point.latitude >= 37.762448 &    # lower lat bound
                          point.latitude <= 37.771545 &    # upper lat bound
                          point.longitude >= -122.434624 &  # upper long bound
                          point.longitude <= -122.4213293)   # lower long bound
...
# 10 cumulative incidence after, (2 wweks).
tmap_mode('view')
tm_shape(odm_duboce) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(odm_dolores) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(marketchurch_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01)


### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(marketchurch_box_bef)
raw_incidence_aft <- st_drop_geometry(marketchurch_box_aft)
