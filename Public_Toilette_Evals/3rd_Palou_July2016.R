...
# SAME start date as SOMA toilette, july 2016 
...
palou_box_aft <- subset(odm_coords_aft, 
                        point.latitude >= 37.7310007302 &    # lower lat bound
                          point.latitude <= 37.7369791222 &    # upper lat bound
                          point.longitude >= -122.3945277397 &  # upper long bound
                          point.longitude <= -122.3872898482)   # lower long bound
...
# 1 cumulative incidence 1 month before.
tmap_mode('view')
tm_shape(odm_bayview) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(palou_box_bef) +
  tm_dots(col = 'red', size = 0.01) 

# 2 cumulative incidence one month after.
tmap_mode('view')
tm_shape(odm_bayview) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(palou_box_aft) +
  tm_dots(col = 'red', size = 0.01) 
...
palou_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                        point.latitude >= 37.7310007302 &    # lower lat bound
                          point.latitude <= 37.7369791222 &    # upper lat bound
                          point.longitude >= -122.3945277397 &  # upper long bound
                          point.longitude <= -122.3872898482)   # lower long bound

palou_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                        point.latitude >= 37.7310007302 &    # lower lat bound
                          point.latitude <= 37.7369791222 &    # upper lat bound
                          point.longitude >= -122.3945277397 &  # upper long bound
                          point.longitude <= -122.3872898482)   # lower long bound
...
tm_shape(odm_bayview) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(palou_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01) 


# NOTE: possible reporting bias, regional bias, double check script?
# Evaluate larger time, social landscape to verify result here?

### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(palou_box_bef)
raw_incidence_aft <- st_drop_geometry(palou_box_aft)
