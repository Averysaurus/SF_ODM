# -122.419986,37.748833,-122.411346,37.754457

### 4.27.20 ###
# odm toilette intervention chunk.
# 24th and Mission, October 1st, 2018 
...
sanfran_calls_no_na_47 <- sanfran_calls_no_na_46[grep("could not find", sanfran_calls_no_na_46$status_notes, invert = TRUE), ]
...
sanfran_mostly_clean = sanfran_calls_no_na_47 %>% 
...
mission24th_box_bef <- subset(odm_coords_bef, 
                        point.latitude >= 37.748833 &    # lower lat bound
                        point.latitude <= 37.754457 &    # upper lat bound
                        point.longitude >= -122.419986 &  # upper long bound
                        point.longitude <= -122.411346)   # lower long bound

# 14 cumulative incidence one month before. 
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(mission24th_box_bef) +
  tm_dots(col = 'red', size = 0.01) 



mission24th_box_aft <- subset(odm_coords_aft, 
                    point.latitude >= 37.748833 &    # lower lat bound
                    point.latitude <= 37.754457 &    # upper lat bound
                    point.longitude >= -122.419986 &  # upper long bound
                    point.longitude <= -122.411346)   # lower long bound
...
# 18 cumulative incidence 1 month after.
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(mission24th_box_aft) +
  tm_dots(col = 'red', size = 0.01) 
...
mission24th_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                      point.latitude >= 37.748833 &    # lower lat bound
                      point.latitude <= 37.754457 &    # upper lat bound
                      point.longitude >= -122.419986 &  # upper long bound
                      point.longitude <= -122.411346)   # lower long bound

# 11 cumulative incidence before. (2 weeks)
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(mission24th_box_bef_twowk) +
  tm_dots(col = 'red', size = 0.01)


mission24th_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                        point.latitude >= 37.748833 &    # lower lat bound
                        point.latitude <= 37.754457 &    # upper lat bound
                        point.longitude >= -122.419986 &  # upper long bound
                        point.longitude <= -122.411346)   # lower long bound
...
# 12 cumulative incidence after, (2 wweks).
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(mission24th_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01)


### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(mission24th_box_bef)
raw_incidence_aft <- st_drop_geometry(mission24th_box_aft)
...
####  use log from SF_DPW data source, tally of services at site ###
setwd("~/Desktop/SF_ODM/data")

# structure as readable date format.
use_log <- read.csv("24_miss_user_log.csv")

use_log$date <- as.Date(use_log$date)

mission_only <- filter(sanfran_map_these_after, neighborhoods_sffind_boundaries == "Mission")

mission_only$date_only <- gsub('[[:upper:]][0-9][^z][[:punct:]][0-9][^z][[:punct:]][0-9][^z][[:upper:]]', '', mission_only$requested_datetime)



mission_only$date_only <- as.Date(mission_only$date_only)

# framing data for by graph.
data_one <- data.frame(x = mission_only$date_only)
data_two <- data.frame(x= use_log$date, y = use_log$user_count)

ggplot()+
  geom_histogram(data = data_one, 
                 aes(x = mission_only$date_only), fill= 'steel blue', alpha = .5, bins = 31) + 
  geom_smooth(data = data_two, aes(x= use_log$date, y = use_log$user_count),
              color = 'red') + coord_trans(x = 'log2', y = 'log2')
  

# seperate plot to compare
hist_plot <- ggplot(data_one, aes(x)) + 
  geom_histogram(fill= 'steel blue', alpha = .5, bins = 31) 
  
line_plot <- ggplot(use_log, aes(x = date, y = user_count)) + 
  geom_smooth(color = 'red')+
  ggtitle("Service Counts at Site")
