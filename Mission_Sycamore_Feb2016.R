# -122.4241867381,37.7598897775,-122.4158178864,37.7653985774

### 4.24.20 ###
# odm toilette intervention chunk.
# Mission & Sycamore, Feb 1st, 2016 
...
## Pulls Mission neighborhood geom for reference.
odm_mission <- sf_nhoods[sf_nhoods$name == 'Mission', ]

# Setting time frame before/after February 1st 2016: 
...
sycamore_box_bef <- subset(odm_coords_bef, 
                          point.latitude >= 37.7598897775 &    # lower lat bound
                          point.latitude <= 37.7653985774 &    # upper lat bound
                          point.longitude >= -122.4241867381 &  # upper long bound
                          point.longitude <= -122.4158178864)   # lower long bound

# 12 cumulative incidence one month before 
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(sycamore_box_bef) +
  tm_dots(col = 'red', size = 0.01) 

sycamore_box_aft <- subset(odm_coords_aft, 
                      point.latitude >= 37.7598897775 &    # lower lat bound
                      point.latitude <= 37.7653985774 &    # upper lat bound
                      point.longitude >= -122.4241867381 &  # upper long bound
                      point.longitude <= -122.4158178864)   # lower long bound

# 12 cumulative incidence one month after. 
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(sycamore_box_aft) +
  tm_dots(col = 'red', size = 0.01) 
...
sycamore_box_bef_twowk <- subset(odm_coords_bef_twowk, 
                      point.latitude >= 37.7598897775 &    # lower lat bound
                      point.latitude <= 37.7653985774 &    # upper lat bound
                      point.longitude >= -122.4241867381 &  # upper long bound
                      point.longitude <= -122.4158178864)   # lower long bound
...
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(sycamore_box_bef_twowk) +
  tm_dots(col = 'red', size = 0.01) 


sycamore_box_aft_twowk <- subset(odm_coords_aft_twowk, 
                        point.latitude >= 37.7598897775 &    # lower lat bound
                        point.latitude <= 37.7653985774 &    # upper lat bound
                        point.longitude >= -122.4241867381 &  # upper long bound
                        point.longitude <= -122.4158178864)   # lower long bound

# 7 cumulative incidence after (2 week)
tmap_mode('view')
tm_shape(odm_mission) +
  tm_polygons(border.col = 'blue', alpha = 0) +
  tm_shape(sycamore_box_aft_twowk) +
  tm_dots(col = 'red', size = 0.01) 


### Visualization: histogram and density functions of incidence 
### plotted over time.
raw_incidence_bef <- st_drop_geometry(sycamore_box_bef)
raw_incidence_aft <- st_drop_geometry(sycamore_box_aft)
...
ggplot(vis_incidence, aes(date_only)) + 
  ...
  ggtitle("density function of incidence over time, before and after")
