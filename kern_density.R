...
# filter tenderloin spots
sanfran_mostly_clean_2 = sanfran_mostly_clean %>% 
  filter(neighborhoods_sffind_boundaries == "Tenderloin")
...
nhoods_3 <- as(as_Spatial(nhoods_2[nhoods_2$name == 'Tenderloin', ]), "owin")
...
Q <- quadratcount(sanfran_ppp, nx= 4, ny=4)
...
Q   <- quadratcount(sanfran_ppp, nx= 20, ny=10)
Q.d <- intensity(Q)
...
Zcut <- cut(sanfran_ppp, breaks=brk, labels=1:4)  # Classify the raster
